...
Meteor.methods({
  helloWorld (data) {
    fs.writeFile('./poop.txt', `the global counter * 2 = ${(data*2).toString()}`, (err)=> {
      if (err) {
         console.log(err)
      } else {
    ...
    let d = fetch('http://api.openweathermap.org/data/2.5/weather?zip=92630,us&APPID=06eb04611b508e74c1a6f0f4e2968ca2')
      .then(d => d.json())
      .then(d => {
