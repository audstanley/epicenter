    ...
    return new Promise((resolve, reject) => {
        ...
        createTorrent(`${process.cwd()}/${filename}`, (err, tor) => {
            if (err) reject(err);
                ...
                // console.log(`${process.cwd()}/${filename}`);
                // console.log(`${process.cwd()}/${storagepath}`)
                // console.log(`${tor.toString('base64')}`)
                // console.log(`${tor}`)
                resolve({'name':`${process.cwd()}/${filename}`,'b64':`${tor.toString('base64')}`, 'tor': tor, 'storagePath': `${process.cwd()}/${storagepath}`  });
            }
        })
    })
}
...
function saveFileToTorrentFolder(obj) {
    return new Promise((resolve, reject) => {
        if (!(obj.name && obj.tor)) reject('saveFileToTransmissionError: { name: string, tor: Buffer}');
        else {
...
function addFileToTransmission(obj) {
    return new Promise((resolve, reject) => {
        if (!(obj.b64 && obj.name)) reject(`addFileToTransmissionError: { b64:string, name: string }`)
        else {
            let torrentPath = `${obj.name}.torrent`;
            // console.log(`torrent path: ${torrentPath}`);
            transmission.addFile(torrentPath, { 'download-dir': `${obj.storagePath}` }, (err, arg) => {
                if (err) reject(err);
                else resolve(obj);
            })
        }
...
// testing the flow:
// createTorrentFromFileName('2019-07-10-raspbian-buster-full.zip')
